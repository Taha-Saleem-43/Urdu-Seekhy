...
class _QuizScreenState extends State<QuizScreen> with TickerProviderStateMixin {
  ...
  _Question _makeQuestion(UrduWord word) {
    ...
    switch (type) {
      case _QType.imageToUrdu:
        ...
        final wrongAny = allW.where((w) => w.urdu != word.urdu).toList()
          ..shuffle(_rng);
        ...
        while (distList.length < needed && wrongAny.length > distList.length) {
          ...
          if (!distList.any((d) => d.urdu == candidate.urdu)) {
            distList.add(candidate);
          }
        }
      ...
      case _QType.urduToEnglish:
        ...
        final wrong = allW2.where((w) => w.english != word.english).toList()
          ..shuffle(_rng);
      ...
      case _QType.fillBlank:
        ...
        final sentences = SENTENCES
            .where((s) =>
                s.words.contains(word.urdu) || s.urdu.contains(word.urdu))
            .toList();
  ...
  void _speakQuestion(_Question q) {
    switch (q.type) {
      case _QType.imageToUrdu:
        unawaited(TtsService.instance.speak('یہ کیا ہے؟'));
        break;
      case _QType.urduToEnglish:
        unawaited(TtsService.instance.speak('${q.word.urdu}۔ اس کا مطلب کیا ہے؟'));
        break;
      case _QType.fillBlank:
        if (q.sentence != null) {
          final blank =
              q.sentence!.urdu.replaceFirst(q.sentence!.blankWord, '۔۔۔');
          unawaited(TtsService.instance.speak('خالی جگہ بھریں: $blank'));
        }
        break;
      case _QType.speakWord:
        unawaited(TtsService.instance.speak('یہ لفظ بولیں: ${q.word.urdu}'));
        break;
    }
  }
  ...
  void _handleCorrect() {
    ...
    unawaited(TtsService.instance.speak('شاباش! بہت اچھے!'));
  ...
  void _speakResult() {
    ...
    if (ratio >= 0.8) {
      unawaited(TtsService.instance.speak('بہت اچھے! آپ نے شاندار کارکردگی دکھائی۔'));
    } else if (ratio >= 0.5) {
      unawaited(TtsService.instance.speak('اچھا کیا! مزید مشق سے اور بہتر ہوں گے۔'));
    } else {
      unawaited(TtsService.instance.speak('ہمت نہ ہاریں! دوبارہ کوشش کریں۔'));
    }
  }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child:
                _phase == _Phase.question ? _buildQuestion() : _buildFeedback(),
          ),
        ],
      ),
    );
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(_current.word.emoji, style: const TextStyle(fontSize: 80)),
          const SizedBox(height: 8),
  ...
  String get _questionLabel {
    switch (_current.type) {
      case _QType.imageToUrdu:
        return 'یہ کیا ہے؟ اردو میں بتائیں';
      case _QType.urduToEnglish:
        return 'اس کا انگریزی مطلب کیا ہے؟';
      case _QType.fillBlank:
        return 'خالی جگہ بھریں';
      case _QType.speakWord:
        return 'یہ لفظ بولیں — مائیکروفون دبائیں';
    }
  }
  ...
  Widget _buildSpeakQuestion() {
    return Column(
      children: [
        ElevatedButton.icon(
          onPressed: () => unawaited(TtsService.instance.speak(_current.word.urdu)),
        ...
        ElevatedButton.icon(
          onPressed: () => unawaited(showModalBottomSheet(
            context: context,
  ...
  Widget _buildFeedback() {
    ...
    final bgColor =
        isCorrect ? const Color(0xFF00C853) : const Color(0xFFD50000);
